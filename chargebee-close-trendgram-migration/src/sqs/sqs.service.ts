...
class SQSService {
  ...
  public async pushToQueue(data: any, queueUrl: string): Promise<void> {
    ...
    try {
      ...
      if (response)
        this.logger.info(`Message sent to SQS:${response?.MessageId}`);
    } catch (error) {
  ...
  async getQueueUrlByName(queueName: string): Promise<string> {
    try {
      ...
      const queueUrl = response.QueueUrls.find((url) =>
      ...
      throw new Error(
        `Failed to fetch queue URL for "${queueName}": ${error.message}`
      );
    }
  }
}
